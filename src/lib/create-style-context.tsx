import { createContext, splitProps, useContext, type ComponentProps, type ValidComponent } from "solid-js"
...
export const createStyleContext = <R extends Recipe>(recipe: R) => {
  ...
  const withProvider = <T extends ValidComponent>(Component: T, slot?: string) => {
    const StyledComponent = (props: PolymorphicProps<T> & VariantProps<R>) => {
      const [variantProps, componentProps] = splitProps(props, ["class", ...recipe.variantKeys]) as unknown as [
        VariantProps<R>,
        PolymorphicProps<T>,
      ]
      ...
      return (
        <StyleContext.Provider value={styles}>
          <Dynamic component={Component} {...componentProps} class={cx(styles?.[slot ?? ""]?.(), props.class)} />
  ...
  const withContext = <T extends ValidComponent>(Component: T, slot?: string): T => {
    ...
    const StyledComponent = (props: PolymorphicProps<T>) => {
      ...
      return <Dynamic component={Component} {...props} class={cx(styles?.[slot]?.(), props.class)} />
    }
